...
file_path = os.path.join(script_dir, '25014 - Camera for Aerospace Situational Awareness - Team Tasks 10_15_24.tsv')
...
# Filter the DataFrame to include only tasks for Sprint 3
df_sprint_3 = df[df['Sprint'] == 'Sprint 3']  # Modify 'Sprint 3' to match the exact sprint name in your data

# Use .loc to avoid the SettingWithCopyWarning
df_sprint_3.loc[df_sprint_3['Assignees'] == 'None', 'Assignees'] = 'Team'

# Debugging print to check unique sprints in Sprint 3
print("Unique Sprints in Sprint 3 data:", df_sprint_3['Sprint'].unique())
...
output_filename = os.path.join(folder_name, 'tasks_by_assignee_sprint_3.xlsx')
...
# Loop through each team member for Sprint 3
for member in team_members:
    # Filter tasks where the member's name is part of the 'Assignees' column (can have multiple people)
    df_member_sprint = df_sprint_3[df_sprint_3['Assignees'].str.contains(member)]
    
    if df_member_sprint.empty:
        print(f"No tasks found for {member} in Sprint 3")
        continue
    else:
        print(f"Processing tasks for {member} in Sprint 3")

    # Write task data to a new sheet for this member and Sprint 3
    sheet_name = f'{member}_Sprint_3'
    ...
    # Access the worksheet for this member and Sprint 3
    ...
    # Calculate the total of "Hours Completed" for the current member and Sprint 3
